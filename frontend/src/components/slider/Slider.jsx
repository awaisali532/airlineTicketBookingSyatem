      ...
      {slides.map((slide, index) => (
        <div key={slide.id} className={`single-slider slide-${slide.id}`}>
          ...
          <div className="container h-100">
            <div className="row align-items-center h-100 mx-0"> {/* Added mx-0 to remove row margins */}
              <div className="col-12 px-3 px-md-4 text-white text-center text-md-left"> {/* Added px-3 for mobile padding */}
                <div className={`slider-content ${currentSlide === index ? "animate" : ""}`}>
                  <h2 className="display-5 display-md-4 fw-bold mb-3">{slide.text}</h2>
                  <p className="mb-4 d-none d-md-block">
                    Get rewarded for your travels – unlock instant savings of 10% or more with a free Geairinfo.com account.
                  </p>
                  <p className="mb-4 d-md-none mx-3"> {/* Added mx-3 for better mobile spacing */}
                    Get rewarded for your travels with instant savings.
                  </p>
                  <Link to={"/login"} className="btn custom_btn btn-lg px-4  ">
                    Sign In / Register
                  </Link>
                </div>
              </div>
